...
begin
  ...
  if not exists (select 1 from pg_constraint where conname = 'trips_kilos_available_non_negative') then
    alter table trips
      add constraint trips_kilos_available_non_negative check (kilos_available >= 0);
  end if;
...
-- Reserve kilos as soon as a booking is created (pending). Search / trip card then shows remaining capacity.
...
-- On accept: kilos already reserved at insert — no change.
-- On pending -> rejected/cancelled: release reserved kilos back to the trip.
-- On accepted -> cancelled/rejected: release kilos (same as before).
...
begin
  ...
  if new.status = 'accepted' and v_traveler_id is not null then
    ...
    values (
      v_traveler_id,
      'booking',
      'Reservation annulee par l expediteur',
      jsonb_build_object('booking_id', new.id, 'status', new.status)
    );
    ...
    values (
      v_traveler_id,
      'booking',
      'Reservation marquee comme terminee',
      jsonb_build_object('booking_id', new.id, 'status', new.status)
    );
    insert into notifications (user_id, type, title, payload)
    values (
